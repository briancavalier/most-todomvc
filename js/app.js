...
// remove :: Int -> [a] -> [a]
// remove element at index
function remove$1$1 (i, a) {  // eslint-disable-line complexity
  if (i < 0) {
    throw new TypeError('i must be >= 0')
  }
...
module.exports = function h(sel, b, c) {
  ...
  if (c !== undefined) {
    data = b;
    if (is$$1.array(c)) { children = c; }
    else if (is$$1.primitive(c)) { text = c; }
  } else if (b !== undefined) {
    if (is$$1.array(b)) { children = b; }
    else if (is$$1.primitive(b)) { text = b; }
    else { data = b; }
  }
  if (is$$1.array(children)) {
    for (i = 0; i < children.length; ++i) {
...
// replace :: a -> Int -> [a]
// replace element at index
function replace$2 (x, i, a) { // eslint-disable-line complexity
  if (i < 0) {
    throw new TypeError('i must be >= 0')
  }
...
var addTodo = function (description) { return function (todos) { return description.trim().length === 0 ? todos
...
var updateDescription = function (id$$1, description) { return function (todos) {
  ...
  return i < 0 ? todos
    : replace$2(newTodo(id$$1, description, todos[i].complete), i, todos)
}; }
...
var updateComplete = function (ref) {
  ...
  var i = findTodoIndex(id$$1, todos)
  return i < 0 ? todos
    : replace$2(newTodo(id$$1, todos[i].description, complete), i, todos)
};
}
...
var renderApp = function (todos, editing, view, ref) {
    ...
    return section('.todoapp', { class: stateClasses(todos.length, remaining, complete, view) }, [
  	renderHeader(),
...
var renderHeader = function () { return header('.header', [
    h1('todos'),
    form('.add-todo', [
      input$1('.new-todo', { attrs: { placeholder: 'What needs to be done?', autofocus: true } })
    ])
  ]); }

var renderTodos = function (todos, editing) { return section('.main', [
    input$1('.toggle-all',
...
var renderTodo = function (editing) { return function (ref) {
    ...
    return li({ class: {completed: complete, editing: editing === id}, attrs: {'data-id': id} }, [
    div('.view', [
      input$1('.toggle', {props: {type: 'checkbox', checked: complete}}),
      label(description),
      button('.destroy')
    ]),
    input$1('.edit', {props: {value: editing === id ? description : ''}})
  ]);
 }  }

var renderFooter = function (remaining) { return footer('.footer', [
    span('.todo-count', [
      strong(remaining),
...
var stateClasses = function (total, remaining, complete, view) { return Object.assign(
...
var findDataId = function (node) { return node == null ? ''
